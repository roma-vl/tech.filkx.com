...
<template>
  <div v-if="store.activeDrawer === 'wishlist'" class="fixed inset-0 z-[90] flex justify-end">
    <!-- Backdrop Overlay -->
    <div
    ...
    >
      ...
      <div class="p-6 border-b border-surface-variant flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <h2 class="font-headline-md text-headline-md text-on-surface">Your Wishlist</h2>
        </div>
        <button
          @click="store.closeDrawer()"
      ...
      <div class="flex-grow overflow-y-auto p-6 flex flex-col gap-4">
        <!-- Empty State -->
        <div v-if="store.wishlist.length === 0" class="flex-grow flex flex-col items-center justify-center text-center gap-4 py-12">
          ...
          <h3 class="font-title-lg text-title-lg text-on-surface">Wishlist is empty</h3>
          <p class="text-on-surface-variant text-xs max-w-[240px]">
            Keep track of premium products you love. Tap the heart icons to save items here.
          </p>
          <button
            @click="store.closeDrawer()"
        ...
        >
          ...
          <div class="flex-grow flex flex-col justify-between pr-6">
            <div>
              ...
              <h4 class="font-title-md text-sm text-on-surface line-clamp-2 leading-tight">{{ item.name }}</h4>
            </div>
            ...
            <div class="flex items-center justify-between mt-2">
              ...
              >
                Add to Cart <span class="material-symbols-outlined text-[14px]">shopping_cart</span>
              </button>
            </div>
          </div>
