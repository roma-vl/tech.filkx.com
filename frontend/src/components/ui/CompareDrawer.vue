  ...
  >
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <div class="text-sm font-semibold">
          Compare Products
          <span class="text-xs text-primary-fixed-dim">({{ store.compare.length }} / 3 selected)</span>
        </div>
    ...
    >
      ...
      >
        <!-- Modal Header -->
        <div
          class="flex justify-between items-center border-b border-surface-variant pb-4"
        >
          ...
          <table class="w-full border-collapse">
            <thead>
              <tr class="border-b border-surface-variant/30">
                <th
                  class="w-1/4 py-4 px-6 text-left font-title-md text-on-surface-variant text-xs uppercase tracking-wider bg-surface-container-low rounded-tl-xl"
                >
                  Spec
                </th>
                <th
                ...
                >
                  ...
                  <h4
                    class="font-title-md text-sm leading-snug line-clamp-2 max-w-[200px] mx-auto"
                  >
                    {{ prod.name }}
                  </h4>
                </th>
                ...
                >
                  <div
                    class="h-24 flex flex-col items-center justify-center border border-dashed border-outline-variant/50 rounded-xl m-2"
                  >
                    <span
                      class="material-symbols-outlined text-[28px] opacity-40 mb-1"
                    >add</span>
            ...
            <tbody>
              ...
              <tr class="border-b border-surface-variant/30">
                <td
                  class="py-4 px-6 font-bold text-xs text-on-surface-variant uppercase bg-surface-container-low/30"
                >
                  Price
                </td>
                <td
                  v-for="prod in store.compare"
                  :key="prod.id"
                  class="py-4 px-6 text-center font-bold text-primary text-lg"
                >
              ...
              <tr class="border-b border-surface-variant/30">
                ...
                <td
                  v-for="prod in store.compare"
                  :key="prod.id"
                  class="py-4 px-6 text-center text-sm font-semibold text-on-surface"
                >
                  {{ prod.category }}
                </td>
                <td
              ...
              <tr class="border-b border-surface-variant/30">
                ...
                >
                  <div class="flex items-center justify-center gap-1">
                    <span
                      class="material-symbols-outlined text-[16px] text-amber-400 fill"
              ...
              <tr class="border-b border-surface-variant/30">
                ...
                <td
                  v-for="prod in store.compare"
                  :key="prod.id"
                  class="py-4 px-6 text-center text-xs text-on-surface-variant leading-relaxed max-w-[240px]"
                >
                  {{ prod.description }}
                </td>
                <td
              ...
              <tr>
                <td
                  class="py-6 px-6 font-bold text-xs text-on-surface-variant bg-surface-container-low/30 rounded-bl-xl"
                />
                <td
                  v-for="prod in store.compare"
                  :key="prod.id"
