...
<template>
  <aside
    class="hidden lg:flex flex-col sticky top-[80px] h-[calc(100vh-80px)] w-72 bg-zinc-50 dark:bg-zinc-955 pt-10 px-6 pb-6 gap-6 border-r border-zinc-200 dark:border-zinc-850"
  >
    <!-- User info -->
    <div
      class="flex flex-col gap-1 pb-5 border-b border-zinc-200 dark:border-zinc-800"
    >
      <div class="flex items-center gap-4">
        <div
          class="w-14 h-14 rounded-full bg-emerald-500/10 text-[#00a046] flex items-center justify-center text-xl font-black border border-emerald-500/20 select-none"
        >
          {{ userInitials }}
        </div>
        <div class="min-w-0">
          <p
            class="font-extrabold text-zinc-900 dark:text-white leading-tight truncate text-base"
          >
            {{ userName }}
          </p>
          <p class="text-xs text-zinc-400 dark:text-zinc-500 truncate mt-1">
            {{ userEmail }}
          </p>
          <div class="flex items-center gap-1.5 mt-1.5">
            <span class="material-symbols-outlined text-[14px] text-[#00a046]">verified</span>
            <p
              class="font-black text-[#00a046] uppercase tracking-widest text-[10px]"
            >
              Клієнт
            </p>
          </div>
    ...
    <nav class="flex flex-col gap-2">
      ...
      >
        ...
        <span
          v-if="isActive(item)"
          class="ml-auto material-symbols-outlined text-[18px]"
        >chevron_right</span>
      </button>
    </nav>
    ...
    <!-- Footer nav -->
    <div
      class="mt-auto flex flex-col gap-2 pt-5 border-t border-zinc-200 dark:border-zinc-800"
    >
      <button
