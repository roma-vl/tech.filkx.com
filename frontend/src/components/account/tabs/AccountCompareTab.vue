...
const formatPrice = (price) => {
  return new Intl.NumberFormat('uk-UA', { style: 'currency', currency: 'UAH', maximumFractionDigits: 0 }).format(price);
};
</script>
...
<template>
  <div class="space-y-6 animate-fade font-sans select-none">
    ...
    <div v-if="!selectedCategory">
      <div v-if="store.compare.length > 0" class="space-y-4">
        <h2 class="text-xl font-black text-zinc-950 dark:text-white tracking-tight mb-6">Списки порівнянь</h2>
        ...
        <div class="grid grid-cols-1 gap-4">
          ...
          >
            <div class="space-y-4 flex-1">
              <div>
                <h3 class="font-extrabold text-base md:text-lg text-zinc-850 dark:text-zinc-150 group-hover:text-[#00a046] transition-colors">
                  {{ catName }}
                </h3>
            ...
            <!-- Actions -->
            <div class="flex items-center gap-3 self-end md:self-center" @click.stop>
              <button 
                ...
                type="button"
                @click="store.addToast('Посилання скопійовано в буфер обміну', 'success')"
              >
      ...
      <!-- Empty state -->
      <div v-else class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-12 text-center shadow-sm">
    ...
    <div v-else class="space-y-6">
      <div class="flex items-center justify-between gap-4">
        ...
        <h2 class="text-lg md:text-xl font-black text-zinc-950 dark:text-white tracking-tight">
          Порівняння: {{ selectedCategory }}
        </h2>
      </div>

      <div class="overflow-x-auto bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
        <table class="w-full min-w-[800px] border-collapse text-left text-sm">
          <thead>
            <tr class="bg-zinc-50 dark:bg-zinc-850/50 border-b border-zinc-200 dark:border-zinc-800 text-zinc-400 dark:text-zinc-500 font-extrabold uppercase text-[10px] tracking-wider">
              <th class="p-5 w-1/4">Параметри</th>
              <th v-for="product in comparedByCategory[selectedCategory]" :key="product.id" class="p-5 relative text-center">
                <button 
                  @click="store.removeFromCompare(product.id); if ((comparedByCategory[selectedCategory] || []).length <= 1) selectedCategory = null;" 
          ...
          <tbody class="divide-y divide-zinc-250/60 dark:divide-zinc-800/60 text-zinc-700 dark:text-zinc-350">
            <!-- Image Row -->
            <tr>
              <td class="p-5 font-extrabold bg-zinc-50/50 dark:bg-zinc-850/20 text-zinc-900 dark:text-white">Зображення</td>
              <td v-for="product in comparedByCategory[selectedCategory]" :key="product.id" class="p-5">
                <div class="flex flex-col gap-3 items-center">
                  <img :src="product.image" :alt="product.name" class="w-20 h-20 object-contain mx-auto bg-white rounded-lg border border-zinc-150 dark:border-zinc-850 p-2" />
                  <h4 class="font-extrabold text-center text-xs md:text-sm line-clamp-2 text-zinc-800 dark:text-zinc-200 max-w-[180px]">{{ product.name }}</h4>
                </div>
              </td>
            </tr>
            <!-- Price Row -->
            <tr>
              <td class="p-5 font-extrabold bg-zinc-50/50 dark:bg-zinc-850/20 text-zinc-900 dark:text-white">Ціна</td>
              <td v-for="product in comparedByCategory[selectedCategory]" :key="product.id" class="p-5 font-black text-[#00a046] text-center text-sm md:text-base">
                {{ formatPrice(product.price) }}
              </td>
            </tr>
            <!-- Rating Row -->
            <tr>
              <td class="p-5 font-extrabold bg-zinc-50/50 dark:bg-zinc-850/20 text-zinc-900 dark:text-white">Рейтинг</td>
              <td v-for="product in comparedByCategory[selectedCategory]" :key="product.id" class="p-5 text-center">
                <div class="flex items-center justify-center gap-1.5 text-amber-400">
                  <span class="material-symbols-outlined text-[16px]" style="font-variation-settings: 'FILL' 1;">star</span>
                  <span class="font-extrabold text-zinc-650 dark:text-zinc-350 text-xs md:text-sm">
            ...
            <!-- Dynamic Specs Rows -->
            <tr v-for="label in dynamicSpecLabels" :key="label">
              <td class="p-5 font-extrabold bg-zinc-50/50 dark:bg-zinc-850/20 text-zinc-900 dark:text-white">{{ label }}</td>
              <td v-for="product in comparedByCategory[selectedCategory]" :key="product.id" class="p-5 text-center text-xs md:text-sm font-semibold text-zinc-800 dark:text-zinc-200">
                {{ getSpecValue(product, label) }}
              </td>
            </tr>
            <!-- Description Row -->
            <tr>
              <td class="p-5 font-extrabold bg-zinc-50/50 dark:bg-zinc-850/20 text-zinc-900 dark:text-white">Опис</td>
              <td v-for="product in comparedByCategory[selectedCategory]" :key="product.id" class="p-5 text-zinc-500 dark:text-zinc-400 text-xs md:text-sm leading-relaxed max-w-[250px] text-center">
                {{ product.description }}
              </td>
            </tr>
            <!-- Actions Row -->
            <tr>
              <td class="p-5 bg-zinc-50/50 dark:bg-zinc-850/20"></td>
              <td v-for="product in comparedByCategory[selectedCategory]" :key="product.id" class="p-5 text-center">
