<template>
  <div
    ...
    :class="{
      ...
      'bg-primary-50/50 dark:bg-primary-900/20 shadow-inner border-primary-200/50 dark:border-primary-800/30':
        isToday,
      'bg-white/20 dark:bg-white/5 backdrop-blur-sm': isPast && !isToday,
      'cursor-pointer hover:bg-white/60 dark:hover:bg-gray-800/40 hover:shadow-md hover:border-white/80 dark:hover:border-white/20 bg-white/30 dark:bg-gray-800/20 backdrop-blur-md':
        !isPast && !isToday,
      'cursor-pointer hover:bg-primary-100/50 dark:hover:bg-primary-900/30 hover:shadow-md hover:border-primary-300/50 dark:hover:border-primary-700/50':
        isToday,
    }"
  ...
  >
    <div class="flex justify-between items-start mb-2 overflow-visible">
      <div
        ...
        :class="{
          'bg-gradient-to-br from-primary-500 to-primary-600 text-white shadow-primary-500/30 scale-110':
            isToday,
          'text-gray-400 dark:text-gray-500 bg-white/40 dark:bg-black/10 border border-white/50 dark:border-white/5':
            isPast && !isToday,
          'text-gray-700 dark:text-gray-200 bg-white/60 dark:bg-white/5 border border-white/60 dark:border-white/10':
            !isPast && !isToday,
        }"
      >
        {{ day.day }}
    ...
    <div
      class="space-y-1.5 custom-scrollbar overflow-y-auto max-h-[80px] sm:max-h-none pr-1"
    >
      <div
      ...
      >
        ...
        />
        <span
          class="font-bold opacity-80 text-gray-500 dark:text-gray-400 tracking-tighter"
        >{{ event.time }}</span>
        <span
          class="font-bold text-gray-700 dark:text-gray-200 truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors"
        >{{ event.title }}</span>
      </div>
    </div>
  </div>
