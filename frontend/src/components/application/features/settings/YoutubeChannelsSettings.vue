<template>
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700/50 p-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
          {{ $t('settings.youtube_channels.description') || 'Manage YouTube channels for automatic streaming' }}
        </p>
      </div>
      <AppButton
        size="sm"
    ...
    <div v-else-if="youtubeStore.channels.length === 0"
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400">
        {{ $t('settings.youtube_channels.no_channels') || 'No YouTube channels connected yet' }}
      </p>
    </div>
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-[10px] text-gray-500 uppercase tracking-widest font-bold mt-0.5">
              {{ channel.channelId }}
            </p>
          </div>
        </div>
...
const confirmDisconnect = async (channel) => {
  if (confirm(t('settings.youtube_channels.disconnect_confirm') || `Disconnect ${channel.title}?`)) {
    try {
      await youtubeStore.disconnectChannel(channel.id);
      toast.success(t('settings.youtube_channels.disconnect_success') || 'Channel disconnected');
    } catch (e) {
      toast.error(t('settings.youtube_channels.disconnect_error') || 'Failed to disconnect channel');
    }
  }
}
