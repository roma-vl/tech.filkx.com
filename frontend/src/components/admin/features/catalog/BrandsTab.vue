<template>
  <div class="space-y-6">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="brand in brands" :key="brand.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex justify-end gap-2">
                  <AppButton @click="openEditBrandModal(brand)" variant="ghost" size="sm" class="!p-2 text-blue-600 dark:text-blue-400">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                  </AppButton>
                  <AppButton @click="deleteBrand(brand.id)" variant="ghost" size="sm" class="!p-2 text-red-600 dark:text-red-400">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    >
      ...
      <template #footer>
        <AppButton variant="secondary" @click="showBrandModal = false" class="mr-2">
          Скасувати
        </AppButton>
        <AppButton @click="saveBrand" variant="primary">
          Зберегти
        </AppButton>
      </template>
    </AppModal>
