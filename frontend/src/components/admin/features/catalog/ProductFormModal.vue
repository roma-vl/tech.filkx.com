<template>
  ...
  >
    <form @submit.prevent="saveProduct" class="space-y-6">
      ...
      <div class="bg-gray-50 dark:bg-gray-900/40 p-5 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-6 pt-2">
          ...
          <div class="flex items-center gap-6 p-4 rounded-xl border border-dashed border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
            <label class="flex items-center gap-2 cursor-pointer select-none text-xs font-bold uppercase tracking-wider text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors">
              <input type="checkbox" v-model="productForm.isHot" class="w-4 h-4 text-primary bg-gray-100 border border-gray-300 rounded focus:ring-primary dark:bg-gray-700 dark:border-gray-600" />
              🔥 Гаряча пропозиція
            </label>
            <label class="flex items-center gap-2 cursor-pointer select-none text-xs font-bold uppercase tracking-wider text-gray-500 hover:text-gray-900 dark:hover:text-white transition-colors">
              <input type="checkbox" v-model="productForm.isRecommended" class="w-4 h-4 text-primary bg-gray-100 border border-gray-300 rounded focus:ring-primary dark:bg-gray-700 dark:border-gray-600" />
              👍 Рекомендовано
            </label>
          </div>
        </div>
      ...
      <div class="space-y-4">
        <div class="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-2">
          <h4 class="font-bold text-gray-900 dark:text-white">2. Варіанти товару та ціни</h4>
          <AppButton type="button" @click="addProductVariant" variant="secondary" size="sm" class="flex items-center gap-1.5">
        ...
        <div v-for="(v, vIdx) in productForm.variants" :key="vIdx" class="bg-gray-50 dark:bg-gray-900/40 p-5 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 space-y-4 relative">
          <div class="absolute top-4 right-4 flex items-center gap-2">
            <span class="text-xs font-black uppercase text-gray-400 bg-gray-200/50 dark:bg-gray-800 px-2 py-0.5 rounded-md">Варіант #{{ vIdx + 1 }}</span>
            <AppButton v-if="productForm.variants.length > 1" type="button" @click="removeProductVariant(vIdx)" variant="ghost" class="!text-red-500 hover:!bg-red-50 dark:hover:!bg-red-900/20 !p-1.5">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4 pt-4">
            <AppInput v-model="v.sku" required label="SKU (Артикул)" placeholder="напр. iphone-15-black" />
            <AppInput v-model.number="v.price" required type="number" step="0.01" label="Ціна (UAH)" placeholder="0.00" />
            <AppInput v-model.number="v.oldPrice" type="number" step="0.01" label="Стара ціна (UAH)" placeholder="0.00" />
            <AppInput v-model.number="v.stock" required type="number" label="Кількість" placeholder="0" />
            <AppInput v-model.number="v.weight" type="number" step="0.001" label="Вага (кг)" placeholder="0.000" />
          </div>

          <!-- Image Upload / Management -->
          <div class="space-y-2">
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider">Зображення варіанту</label>
            <div class="flex flex-wrap items-center gap-4">
              <!-- Upload Image Button -->
              <label class="flex flex-col items-center justify-center w-24 h-24 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl cursor-pointer hover:border-primary-500 dark:hover:border-primary-500 hover:bg-gray-100/50 dark:hover:bg-gray-800/35 transition-all duration-200">
                ...
                <input type="file" class="hidden" accept="image/*" multiple @change="onFileChange($event, v)" />
              </label>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
                ...
                >
                  ...
                  <button type="button" @click="removeVariantImage(v, imgIdx)" class="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200 shadow hover:bg-red-600">
          ...
          <div class="space-y-3 pt-2">
            <div class="flex justify-between items-center">
              <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider">Характеристики варіанту</label>
              <AppButton type="button" @click="addVariantAttribute(v)" variant="ghost" size="sm" class="!text-primary-600 hover:!bg-primary-50 dark:hover:!bg-primary-950/20 !px-2.5 !py-1 !text-xs font-bold">
                + Додати характеристику
              </AppButton>
            </div>

            <div v-if="v.attributes.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              <div v-for="(attr, attrIdx) in v.attributes" :key="attrIdx" class="flex items-end gap-2 bg-white dark:bg-gray-900 p-3 rounded-xl border border-gray-150 dark:border-gray-800 shadow-xs relative group/attr">
                ...
                <AppButton type="button" @click="removeVariantAttribute(v, attrIdx)" variant="ghost" class="!text-red-500 hover:!bg-red-50 dark:hover:!bg-red-900/20 !p-1.5 mb-1 select-none">
                  <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                  </svg>
                </AppButton>
    ...
    <template #footer>
      <AppButton variant="secondary" @click="$emit('update:modelValue', false)" class="mr-2">
        Скасувати
      </AppButton>
