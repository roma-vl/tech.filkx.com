<template>
  <div
    class="max-w-2xl mx-auto bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 md:p-12 text-center shadow-lg my-12"
  >
    <div
      class="w-20 h-20 mx-auto rounded-full bg-[#00a046]/10 flex items-center justify-center text-[#00a046] mb-6"
    >
...
// TODO: these are placeholders - fill in the real registered bank account
// once available. Do not show customers an account number that isn't real.
const bankDetails = {
  iban: "[IBAN]",
  edrpou: "[ЄДРПОУ]",
};
...
const paymentPurpose = computed(() =>
  t("cart.success.purposeValue", {
    number:
      props.orderSuccessData?.orderNumber ||
      props.orderSuccessData?.order_number,
  }),
);
...
function getPaymentMethodLabel(method?: string) {
  if (!method) return "";
  if (method === "cod") return t("cart.paymentMethods.cod");
  if (method === "card") return t("cart.paymentMethods.card");
  return t("cart.paymentMethods.bank");
}
</script>
