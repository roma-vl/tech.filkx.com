<template>
  <section class="mt-14">
    <!-- Tab bar -->
    <div
      class="flex border-b border-zinc-200 dark:border-zinc-800 mb-10 overflow-x-auto hide-scrollbar"
    >
      <button
        v-for="tab in tabs"
        :key="tab.id"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
      ...
      <div class="lg:col-span-8">
        ...
        <section v-else-if="activeTab === 'specs'" class="space-y-5 text-left">
          <h3
            class="text-lg font-extrabold text-zinc-900 dark:text-white flex items-center gap-2"
          >
            ...
            <table class="w-full text-left border-collapse">
              <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                <tr
                  ...
                  :key="spec[0]"
                  :class="
                    Number(index) % 2 ? 'bg-zinc-50/50 dark:bg-zinc-800/20' : ''
                  "
                  class="hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10 transition-colors"
                >
                  <td
                    class="px-5 py-3.5 text-sm font-semibold text-zinc-500 dark:text-zinc-400 w-2/5"
                  >
                    {{ spec[0] }}
                  </td>
                  <td
                    class="px-5 py-3.5 text-sm font-bold text-zinc-800 dark:text-zinc-200"
                  >
                    {{ spec[1] }}
                  </td>
                </tr>
        ...
        >
          ...
          <div v-if="reviewsLoading" class="space-y-4">
            <div
              v-for="i in 2"
              :key="i"
              class="p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl animate-pulse"
            >
              <div class="flex gap-3 mb-3">
                <div
                  class="w-8 h-8 bg-zinc-200 dark:bg-zinc-800 rounded-full"
                />
          ...
          <template v-else>
            ...
            >
              ...
              >
                <p
                  class="text-5xl font-extrabold text-zinc-900 dark:text-white tracking-tighter"
                >
                  {{ reviewStats.avg }}
                </p>
                <div class="flex justify-center text-amber-400">
                  <span
                    ...
                    :style="
                      star <= Math.round(reviewStats.avg)
              ...
              <div
                class="md:col-span-8 p-6 border border-zinc-200 dark:border-zinc-800 rounded-xl flex flex-col justify-center gap-3"
              >
                <div
                ...
                >
                  ...
                  <span
                    class="font-semibold w-7 text-zinc-400 dark:text-zinc-500 shrink-0 text-right"
                    >{{ count }}</span
                  >
                </div>
              </div>
            ...
            <div v-if="liveReviews.length > 0" class="space-y-4">
              ...
              >
                ...
                <p
                  class="text-[15px] text-zinc-600 dark:text-zinc-400 leading-relaxed"
                >
                  {{ review.body }}
                </p>
            ...
            <div v-else class="py-12 text-center">
              ...
              >
                <span
                  class="material-symbols-outlined text-[28px] text-zinc-400"
                  >rate_review</span
                >
              </div>
        ...
        <section v-else class="grid grid-cols-1 md:grid-cols-3 gap-5 text-left">
          ...
          >
            <span
              class="material-symbols-outlined text-[#00a046] text-[26px]"
              >{{ item.icon }}</span
            >
      ...
      <aside class="lg:col-span-4 space-y-5 text-left">
        <div
          class="p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl space-y-5"
        >
          <h4
            class="text-xs font-extrabold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 flex items-center gap-2"
          >
          ...
          <ul class="space-y-4">
            ...
            >
              <span
                class="material-symbols-outlined text-[#00a046] text-[20px] mt-0.5 shrink-0"
                >{{ item.icon }}</span
              >
              <div>
                ...
                <p
                  class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5 leading-relaxed"
                >
                  {{ item.text }}
                </p>
              </div>
        ...
        <div
          class="p-5 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-700/20 rounded-xl space-y-3"
        >
  ...
  <Teleport to="body">
    ...
    >
      ...
      <div
        v-if="lightbox.photos.length > 1"
        class="absolute bottom-4 left-1/2 -translate-x-1/2 text-white/70 text-xs font-semibold"
      >
        {{ lightbox.index + 1 }} / {{ lightbox.photos.length }}
      </div>
    </div>
...
const fetchReviews = async () => {
  ...
  try {
    const { data } = await api.get(
      `/v1/catalog/products/${props.product.slug}/reviews`,
    );
