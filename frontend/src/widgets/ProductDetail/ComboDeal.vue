<template>
  <section v-if="bundleItems.length > 1" class="mt-14">
    <div
      class="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-3"
    >
      <div>
        <span
    ...
    <div
      class="bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-5 md:p-6 flex flex-col lg:flex-row lg:items-center gap-8"
    >
      <!-- Items list -->
      <div
        ...
        <template v-for="(item, index) in bundleItems" :key="item.id">
          <div
            ...
            :class="{
              ...
              'border-zinc-100 dark:border-zinc-700 opacity-55':
                !item.locked && !selectedBundleIds.includes(item.id),
              'cursor-pointer hover:border-[#00a046]/30 hover:opacity-100 select-none':
                !item.locked,
            }"
          ...
          >
            ...
            <div
              class="aspect-square bg-zinc-50 dark:bg-zinc-700 rounded-lg p-2 flex items-center justify-center mb-3"
            >
              <img
                :alt="item.name"
                class="max-h-[72px] object-contain"
                :src="item.image"
              />
            </div>
            <p
              class="text-[11px] font-bold text-zinc-800 dark:text-zinc-200 truncate"
            >
              {{ item.name }}
            </p>
      ...
      <!-- Summary -->
      <div
        class="lg:w-64 border-t lg:border-t-0 lg:border-l border-zinc-200 dark:border-zinc-700 pt-5 lg:pt-0 lg:pl-6 space-y-4"
      >
