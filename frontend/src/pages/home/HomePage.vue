...
const mapProduct = (p) => {
  try {
    ...
    const category = p.categories?.[0]?.name?.uk || p.categories?.[0]?.name?.en || p.categories?.[0]?.name || 'Товар';
    ...
    let image = 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=500&fit=crop';
    ...
    prodAttrVals.slice(0, 4).forEach(av => {
      const label = av.attribute?.name?.uk || av.attribute?.name?.en || av.attribute?.name || '';
      const attrValObj = av.attributeValue || av.attribute_value;
      const val = attrValObj?.value?.uk || attrValObj?.value?.en || attrValObj?.value || av.customValue || av.custom_value || '';
      if (label && val) {
...
onMounted(async () => {
  try {
    ...
    if (data && (data.success || data.status === 'success')) {
      ...
      recommended.value = (data.data.recommended || []).map(mapProduct).filter(Boolean);
    }
  } catch (error) {
...
<template>
  <main class="flex-grow pb-8 bg-zinc-50/50 dark:bg-zinc-950/50">
    ...
    <div v-if="loading" class="max-w-container-max mx-auto px-4 md:px-8 py-10 space-y-16">
      <!-- Loading Skeleton for Categories -->
      <div class="space-y-6">
        <div class="h-8 w-48 bg-zinc-200 dark:bg-zinc-800 rounded animate-pulse"></div>
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-5">
          <div v-for="i in 8" :key="i" class="h-32 bg-zinc-200 dark:bg-zinc-800 rounded-2xl animate-pulse"></div>
        </div>
      </div>
      <!-- Loading Skeleton for Flash Deals -->
      <div class="space-y-6">
        <div class="h-8 w-64 bg-zinc-200 dark:bg-zinc-800 rounded animate-pulse"></div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div v-for="i in 4" :key="i" class="h-[450px] bg-zinc-200 dark:bg-zinc-800 rounded-3xl animate-pulse"></div>
        </div>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <FlashDeals v-if="flashDeals.length > 0" :products="flashDeals" />
      <RecommendedProducts v-if="recommended.length > 0" :products="recommended" />
    </template>
