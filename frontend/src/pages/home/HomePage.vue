    ...
    <template v-if="loading">
      <div class="max-w-container-max mx-auto px-4 md:px-8 py-6 space-y-6">
        ...
        <div class="space-y-3">
          ...
          <div class="hidden lg:flex items-center gap-3">
            ...
            <div class="flex-1 flex gap-2">
              <div v-for="i in 6" :key="i" class="h-10 w-28 bg-zinc-200 dark:bg-zinc-800 rounded-lg animate-pulse shrink-0" />
            </div>
          </div>
          <!-- Slider skeleton -->
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div v-for="i in 4" :key="`usp-${i}`" class="h-20 bg-zinc-200 dark:bg-zinc-800 rounded-2xl animate-pulse" />
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
            ...
            <div class="p-5 pb-4 border-b border-zinc-100 dark:border-zinc-800 flex gap-2">
              <div v-for="i in 10" :key="i" class="h-16 w-14 bg-zinc-100 dark:bg-zinc-800 rounded-xl animate-pulse shrink-0" />
            </div>
            <!-- Best sellers area -->
            <div class="flex">
        ...
        <div class="space-y-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <div v-for="i in 4" :key="`deal-${i}`" class="h-[450px] bg-zinc-200 dark:bg-zinc-800 rounded-3xl animate-pulse" />
          </div>
        </div>
      </div>
    </template>
