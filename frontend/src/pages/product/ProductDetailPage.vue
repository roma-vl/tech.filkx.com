...
const product = computed(() => {
  if (rawProduct.value) {
    ...
    const getAttrValue = (code) => {
      ...
      return (
        match.customValue ||
        match.attributeValue?.value?.uk ||
        match.attributeValue?.value?.en ||
...
const availableColors = computed(() => {
  ...
  rawProduct.value.variants.forEach((v) => {
    const colorAttr = v.attributeValues?.find(
      (av) => av.attribute?.code === "color",
    );
    if (colorAttr) {
      const val =
        colorAttr.customValue ||
        colorAttr.attributeValue?.value?.uk ||
        colorAttr.attributeValue?.value?.en;
...
const availableStorage = computed(() => {
  ...
  rawProduct.value.variants.forEach((v) => {
    const storageAttr = v.attributeValues?.find(
      (av) =>
        av.attribute?.code === "storage" || av.attribute?.code === "memory",
    );
    if (storageAttr) {
      const val =
        storageAttr.customValue ||
        storageAttr.attributeValue?.value?.uk ||
        storageAttr.attributeValue?.value?.en;
...
const selectVariantByAttributes = (attributeCode, value) => {
  ...
  const matchedVariant = rawProduct.value.variants.find((v) => {
    ...
    const val =
      attr.customValue ||
      attr.attributeValue?.value?.uk ||
      attr.attributeValue?.value?.en;
    return val === value;
  });
...
watch(
  ...
  (newVariant) => {
    ...
    const getAttrValue = (code) => {
      const match = newVariant.attributeValues?.find(
        (av) => av.attribute?.code === code,
      );
      if (!match) return null;
      return (
        match.customValue ||
...
<template>
  ...
  <div v-else>
    ...
    >
      <!-- Breadcrumbs -->
      <nav
        class="flex items-center gap-1.5 text-xs text-zinc-400 dark:text-zinc-550 mb-8 font-bold"
      >
        <a
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-7 flex flex-col md:flex-row gap-4">
          ...
          >
            ...
            <div
              class="absolute bottom-4 left-4 bg-zinc-950/80 backdrop-blur-sm border border-white/10 rounded-lg px-2.5 py-1 text-[9px] font-black uppercase tracking-wider text-white"
            >
        ...
        <div class="lg:col-span-5 flex flex-col gap-6">
          <div class="space-y-4">
            <div class="flex justify-between items-center">
              ...
              <div class="flex gap-2">
                ...
                <button
                  ...
                  type="button"
                  :class="{
                    'text-[#00a046] border-[#00a046]/40': store.isInCompare(
                      product.id,
                    ),
                  }"
            ...
            <h1
              class="text-2xl md:text-3xl font-black text-zinc-900 dark:text-white tracking-tight leading-tight"
            >
              {{ product.name }}
            </h1>
            <p class="text-sm text-zinc-450 dark:text-zinc-500 font-bold">
              {{ product.subtitle }}
            </p>
            ...
            <div class="flex flex-wrap items-center gap-4 py-1 text-xs">
              ...
              <span
                class="font-bold text-zinc-450 dark:text-zinc-500 uppercase tracking-wider"
              >КОД: FLX-{{ product.id }}</span>
            </div>
          </div>

          <!-- Price box -->
          <div
            class="p-6 bg-zinc-50 dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-xl space-y-6"
          >
            <div class="space-y-2">
              <div class="flex flex-wrap items-baseline gap-3">
                <span
                  class="text-3xl font-black tracking-tight text-[#00a046]"
                ...
                <span
                  v-if="product.oldPrice"
                  class="bg-rose-600 text-white text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-wider ml-auto"
                >Акція</span>
              </div>
              <p class="text-[11px] text-zinc-450 dark:text-zinc-500 font-bold">
            ...
            >
              <span
                class="font-black text-xs uppercase tracking-wider text-zinc-450 dark:text-zinc-500"
                ...
                <button
                  ...
                  :class="
                    selectedStorage === storage
                      ...
                      : 'border border-zinc-250 dark:border-zinc-800 font-bold text-zinc-550 dark:text-zinc-400 hover:border-[#00a046] hover:text-[#00a046]'
                  "
            ...
            <div class="pt-2 space-y-3">
              <button
                class="w-full bg-[#00a046] hover:bg-[#00b050] text-white py-3.5 rounded-lg font-extrabold text-sm shadow-md hover:shadow-lg hover:shadow-emerald-500/10 active:scale-[0.98] transition-all flex items-center justify-center gap-2 uppercase tracking-wider"
                type="button"
              ...
              <button
                class="w-full border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-55 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-200 py-3.5 rounded-lg font-extrabold text-sm active:scale-[0.98] transition-all uppercase tracking-wider"
                type="button"
                @click="store.addToCart(product)"
              >
                Швидке замовлення
            ...
            <!-- Delivery info -->
            <div
              class="grid grid-cols-2 gap-px bg-zinc-200/50 dark:bg-zinc-800 rounded-lg overflow-hidden border border-zinc-200/50 dark:border-zinc-800"
            >
              <div
                class="bg-white dark:bg-zinc-900 p-4.5 flex flex-col gap-1 items-center text-center"
              >
                <span
                  class="material-symbols-outlined text-[#00a046] text-[22px]"
                >inventory_2</span>
                <span
                  class="text-[10px] font-black text-[#00a046] tracking-wider uppercase mt-1"
                >В наявності</span>
              ...
              >
                ...
                >local_shipping</span>
                <span
                  class="text-[10px] font-black text-zinc-700 dark:text-zinc-300 tracking-wider uppercase mt-1"
                >Доставка</span>
      ...
      <section class="mt-16">
        ...
        >
          <div>
            ...
            <p class="text-xs text-zinc-450 dark:text-zinc-500 mt-2">
        ...
        <div
          class="bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-150 dark:border-zinc-800 p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-8"
        >
          <!-- Bundle items list -->
          <div
          ...
          >
            ...
            >
              ...
              >
                ...
                <div class="space-y-1">
                  <p
                    class="text-[10px] font-black uppercase truncate text-zinc-800 dark:text-zinc-200"
                  >
                    {{ item.name }}
                  </p>
                  <p
                  ...
                  <p class="text-xs font-black text-[#00a046]">
          ...
          <div
            class="lg:w-72 border-t lg:border-t-0 lg:border-l border-zinc-200 dark:border-zinc-800 pt-6 lg:pt-0 lg:pl-8 space-y-4"
          >
            <div class="space-y-1">
              <p
                class="text-[10px] font-black uppercase tracking-wider text-zinc-450 dark:text-zinc-500"
              >
                Загальна вартість:
              </p>
              <p class="text-2xl font-black text-[#00a046] tracking-tight">
              ...
              <p
                class="bg-rose-100 dark:bg-rose-950/30 text-rose-600 dark:text-rose-450 font-black text-[9px] px-2 py-0.5 rounded inline-block tracking-wider uppercase"
              >
                Економія: {{ formatPrice(bundleSavings) }}
              </p>
            </div>
            <button
              class="w-full bg-[#00a046] hover:bg-[#00b050] text-white px-6 py-3 rounded-lg font-extrabold text-xs transition-all uppercase tracking-wider shadow-sm"
              type="button"
      ...
      <section class="mt-16">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div class="lg:col-span-8">
            ...
            >
              <div class="max-w-2xl space-y-4">
                <h3
                  class="text-xl md:text-2xl font-black text-zinc-900 dark:text-white tracking-tight leading-snug"
                >
                  Опис товару
                </h3>
                <p
                  class="text-sm text-zinc-550 dark:text-zinc-400 leading-relaxed whitespace-pre-line"
                >
                  {{ product.description }}
                </p>
              </div>
            ...
            >
              <h3
                class="font-extrabold text-lg text-zinc-900 dark:text-white flex items-center gap-2"
              >
                <span class="material-symbols-outlined text-[#00a046]">terminal</span>
                Технічні характеристики
              </h3>
              <div
                class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-150 dark:border-zinc-800 overflow-hidden shadow-sm"
              >
                <table
                ...
                >
                  <thead>
                    <tr
                      class="bg-zinc-50 dark:bg-zinc-850 border-b border-zinc-150 dark:border-zinc-800"
                    >
                      <th
                  ...
                  <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    <tr
                      ...
                      :key="spec[0]"
                      :class="
                        index % 2 ? 'bg-zinc-50/30 dark:bg-zinc-850/10' : ''
                      "
                    ...
                    >
                      ...
                      <td
                        class="p-4 text-zinc-600 dark:text-zinc-400 font-extrabold"
                      >
                        {{ spec[1] }}
                      </td>
                    </tr>
            ...
            >
              <div class="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
                <div
                  class="md:col-span-4 p-6 bg-zinc-50 dark:bg-zinc-900 rounded-xl border border-zinc-150 dark:border-zinc-800 text-center space-y-2"
                >
                  <p
                    class="text-5xl font-black text-zinc-900 dark:text-white tracking-tighter"
                  >
                    {{ product.rating }}
                  </p>
                ...
                <div
                  class="md:col-span-8 p-6 border border-zinc-150 dark:border-zinc-800 rounded-xl flex flex-col justify-center space-y-3"
                >
                  <div
                    v-for="rating in [
                  ...
                  >
                    <span
                      class="font-extrabold w-12 text-right text-zinc-600 dark:text-zinc-400"
                    >{{ rating.label }}</span>
                    <div
                    ...
                    <span class="font-extrabold w-10 text-zinc-400">{{
                      rating.value
                    }}</span>
                  </div>
                </div>
              ...
              <div class="space-y-4">
                <article
                  ...
                  :key="review.name"
                  class="p-5 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-xl shadow-sm space-y-3"
                >
                  <div class="flex items-center justify-between gap-4">
                    <div>
                      <h4
                        class="font-black text-sm text-zinc-900 dark:text-white leading-snug"
                      >
                        {{ review.title }}
                      </h4>
                      <p
                  ...
                  <p
                    class="text-xs md:text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed"
                  >
                    {{ review.text }}
                  </p>
                </article>
            ...
            >
              <div
                class="p-6 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-xl shadow-sm space-y-3 text-center md:text-left"
              >
                <span
                  class="material-symbols-outlined text-[#00a046] text-[28px]"
                >support_agent</span>
                <h3 class="font-black text-sm text-zinc-900 dark:text-white">
          ...
          <aside class="lg:col-span-4 space-y-6">
            <div
              class="p-6 bg-white dark:bg-zinc-900 border border-zinc-150 dark:border-zinc-800 rounded-xl shadow-sm space-y-5"
            >
              <h4
                class="font-black text-[10px] uppercase tracking-wider text-zinc-450 dark:text-zinc-500 flex items-center gap-2"
              >
                <span
              ...
              <ul class="space-y-4 text-xs">
                ...
                >
                  ...
                  <div class="space-y-0.5">
                    <p class="font-black text-zinc-850 dark:text-zinc-100">
                      {{ item.title }}
                    </p>
                    <p class="text-zinc-500 dark:text-zinc-450">
                      {{ item.text }}
                    </p>
                  </div>
                </li>
            ...
            >
              <div class="relative z-10 space-y-3">
                <h4
                  class="font-black text-sm uppercase tracking-wider text-[#00a046]"
                >
                  Технічний радник
                </h4>
                <p
                ...
                <button
                  class="w-full bg-[#00a046] hover:bg-[#00b050] text-white py-2.5 rounded-lg font-extrabold text-xs flex items-center justify-center gap-1.5 transition-all shadow-sm"
                  type="button"
                >
    ...
    >
      ...
      >
        <div class="flex items-center gap-4 overflow-hidden min-w-0">
          ...
          <div class="truncate">
            <p
              class="font-black text-zinc-900 dark:text-white text-sm truncate tracking-tight"
            >
              {{ product.name }}
            </p>
            <div class="flex items-center gap-3 text-xs mt-0.5">
              <span class="text-[#00a046] font-black">{{
                formatPrice(product.price)
              }}</span>
              <span
                class="text-[9px] font-black text-white bg-[#00a046] px-1.5 py-0.5 rounded uppercase tracking-wider"
              >В наявності</span>
            </div>
          </div>
        </div>
        <button
          class="bg-[#00a046] hover:bg-[#00b050] text-white px-6 py-3 rounded-lg font-extrabold text-xs shadow-md transition-all active:scale-[0.98] shrink-0 uppercase tracking-wider"
          type="button"
          @click="store.addToCart(product)"
        >
          Додати в кошик
