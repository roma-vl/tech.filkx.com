  ...
  <div class="space-y-6 animate-in fade-in duration-500">
    ...
    <div v-if="activeTab === 'posts'" class="space-y-6">
      ...
      <div v-else-if="posts.length === 0" class="text-center py-20 text-gray-400">
        ...
        <p class="text-sm">Постів не знайдено</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        ...
        >
          ...
          <div class="relative h-44 bg-gradient-to-br from-emerald-500/20 to-teal-500/20 overflow-hidden">
            ...
            <!-- Status badge -->
            <span :class="statusBadgeClass(post.status)" class="absolute top-3 left-3 text-xs font-bold px-2.5 py-1 rounded-lg">
          ...
          <div class="p-4 space-y-3">
            ...
            <p v-if="post.excerptUk || post.excerptEn" class="text-xs text-gray-500 dark:text-gray-400 line-clamp-2">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
        <table class="w-full text-sm">
          <thead class="bg-gray-50 dark:bg-gray-700/50">
            <tr>
              <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Назва (УК)</th>
              <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Назва (EN)</th>
              <th class="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Slug</th>
              <th class="px-5 py-3.5 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Пости</th>
              <th class="px-5 py-3.5 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Дії</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            <tr v-if="categories.length === 0">
              <td colspan="5" class="px-5 py-10 text-center text-gray-400 text-sm">Категорій немає</td>
            </tr>
            <tr v-for="cat in categories" :key="cat.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
              <td class="px-5 py-3.5 font-medium text-gray-800 dark:text-gray-200">{{ cat.nameUk }}</td>
              <td class="px-5 py-3.5 text-gray-600 dark:text-gray-400">{{ cat.nameEn }}</td>
              <td class="px-5 py-3.5 text-gray-500 dark:text-gray-500 font-mono text-xs">{{ cat.slug }}</td>
              ...
              <td class="px-5 py-3.5 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button @click="openCategoryModal(cat)" class="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 hover:text-emerald-600 transition-colors">
                    <PencilIcon class="w-3.5 h-3.5" />
                  </button>
                  <button @click="deleteCategory(cat)" class="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-400 hover:text-red-500 transition-colors">
                    <TrashIcon class="w-3.5 h-3.5" />
                  </button>
                </div>
              </td>
      ...
      <div class="flex flex-wrap gap-3">
        ...
        >
          ...
          <div class="flex items-center gap-1 ml-1">
            <button @click="openTagModal(tag)" class="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-emerald-600 transition-colors">
              <PencilIcon class="w-3 h-3" />
            </button>
            <button @click="deleteTag(tag)" class="p-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-400 hover:text-red-500 transition-colors">
              <TrashIcon class="w-3 h-3" />
            </button>
          </div>
        </div>
        <div v-if="tags.length === 0" class="text-sm text-gray-400 py-4">Тегів немає</div>
      </div>
    </div>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showPostModal" class="fixed inset-0 z-[9999] flex">
          <!-- Backdrop -->
          <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" @click="closePostModal" />
          ...
          <div class="relative ml-auto w-full max-w-6xl h-full bg-gray-50 dark:bg-gray-900 overflow-y-auto shadow-2xl">
            ...
            <div class="sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between">
              <div class="flex items-center gap-3">
                <button @click="closePostModal" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-400">{{ postForm.status === 'published' ? 'Опублікований' : postForm.status === 'draft' ? 'Чернетка' : 'Архів' }}</p>
                </div>
              </div>
              ...
              <div class="p-6 space-y-5 overflow-y-auto">
                ...
                <div>
                  ...
                  <div class="relative">
                    ...
                    >
                      <img :src="postForm.coverImage" class="w-full h-full object-cover" />
                      <button
                    ...
                    <label class="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl cursor-pointer hover:border-emerald-400 hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10 transition-colors">
                      ...
                      <input type="file" accept="image/*" class="sr-only" @change="uploadCover" />
                    </label>
                  </div>
                </div>
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="showCategoryModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
            ...
            <div class="space-y-4">
              <div>
                ...
                <input v-model="categoryForm.nameUk" type="text" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1.5">Name (EN) *</label>
                <input v-model="categoryForm.nameEn" type="text" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1.5">Опис (УК)</label>
                <textarea v-model="categoryForm.descriptionUk" rows="2" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1.5">Description (EN)</label>
                <textarea v-model="categoryForm.descriptionEn" rows="2" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1.5">Порядок</label>
                <input v-model.number="categoryForm.order" type="number" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
            </div>
            <div class="flex justify-end gap-3 pt-2">
              <button @click="showCategoryModal = false" class="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                Скасувати
              </button>
              <button @click="saveCategory" :disabled="savingCategory" class="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold transition-colors disabled:opacity-50">
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="showTagModal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" @click="showTagModal = false" />
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-5">
            ...
            <div class="space-y-4">
              <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1.5">Назва (УК) *</label>
                <input v-model="tagForm.nameUk" type="text" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-gray-500 mb-1.5">Name (EN) *</label>
                <input v-model="tagForm.nameEn" type="text" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              </div>
            </div>
            <div class="flex justify-end gap-3 pt-2">
              <button @click="showTagModal = false" class="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                Скасувати
              </button>
              <button @click="saveTag" :disabled="savingTag" class="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold transition-colors disabled:opacity-50">
