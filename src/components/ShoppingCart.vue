...
<template>
  <main class="max-w-container-max mx-auto px-margin-desktop py-stack-lg min-h-screen">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-stack-lg">
      ...
      <button class="text-primary font-bold text-sm hover:underline flex items-center gap-1" type="button" @click="store.currentPage = 'catalog'">
    ...
    <div v-if="store.cart.length === 0" class="bg-surface-container-lowest rounded-xl border border-outline-variant p-12 text-center">
      ...
      <button class="bg-primary text-on-primary px-8 py-3 rounded-lg font-bold hover:bg-primary-container transition-all" type="button" @click="store.currentPage = 'catalog'">
        Explore Catalog
      </button>
    </div>
    ...
    <div v-else class="cart-layout">
      ...
      <aside>
        <div class="bg-surface-container-low rounded-xl p-6 border border-outline-variant sticky top-24">
          ...
          <button class="w-full py-4 bg-primary-container text-on-primary-container rounded-xl font-headline-md text-headline-md shadow-md hover:opacity-90 active:scale-95 transition-all mb-4" type="button">
            Proceed to Checkout
          </button>
